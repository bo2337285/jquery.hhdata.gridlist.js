...
(function($){
	...
	$.fn.gridList = function(option){
		...
		var bindGridEvent = function () {
			...
			.on('click','.grid-col-prompt',function (e) {
			...
			.on('click','.grid-col-prompt-trigger',function (e) {
				if(e.stopPropagation){
            e.stopPropagation();
        }else{
            e.cancelBubble = true;
      ...
      clearExtData();
		...
		GridList.prototype.setPage = function(option){
			var _self = grid,
				pageOpt = _self.page
			_self.page = $.extend({},pageOpt,option);
			setPageVal();
			loadData();
		}
		...
		GridList.prototype.loadData = function(reSize,isRePage){//更新grid的table参数,重新加载数据
			var _self = grid;
			_self.page.sizeFresh=reSize||0;
			if(isRePage){//是否需要重置表格页码
			  _self.page = $.extend({},_self.page,{pageNumber:1});
	      setPageVal();
			}
			loadData();
		};
		...
		GridList.prototype.updateData = function(data,isRePage){
			if(!data){
				console.log("no data");
				return false;
			}
			...
			_self.data = data;
			if (isRePage) {//是否要重置分页值
				_self.page = $.extend({}, _self.page,{pageNumber:data.length>0?1:0});
				setPageVal();
			}
			loadData();
		};
